...
#define FSIZE (4ULL*1024*1024*1024)
#define IOSIZE (2ULL*1024*1024*1024)
...
void performExperiment(int operation, uint32_t datasize, int seq, char *buf, int fd) {
    ...
    if(operation == 0) {
        ...
        {
            if(read_size = (pread64(fd, buf, datasize, offset)) != datasize) {
                perror("read failed\n");
                printf("%s: read size = %d, offset = %llu\n", __func__, read_size, offset);
                exit(-1);
            }
            offset += datasize;
...
/* Example usage: ./a.out write rand 256 */
/*                ./a.out write seq 256 */
